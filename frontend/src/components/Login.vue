...
const passwordInputRef = ref<{ inputRef: HTMLInputElement | null } | null>(null);
...
function validateEmailField() {
    ...
    if (!regex.test(loginForm.value.email)) {
        setWarningMessage(emailInputRef.value?.inputRef, "Email field is improper format");
        return false;
    }
    return true;
}
...
function validatePasswordField() {
    if (loginForm.value.password === null || loginForm.value.password === "") {
        setWarningMessage(passwordInputRef.value?.inputRef, "Password field is required");
        return false;
    }
    if (loginForm.value.password.length < 6) {
        setWarningMessage(passwordInputRef.value?.inputRef, "Password must be longer than 6 characters");
        return false;
    }
    return true;
}
...
function validateConfirmPasswordField() {
    if (loginForm.value.confirmPassword === null || loginForm.value.confirmPassword === "") {
        setWarningMessage(confirmPasswordInputRef.value?.inputRef, "Confirm password field is required");
        return false;
    }
    if (loginForm.value.confirmPassword.length < 6) {
        setWarningMessage(confirmPasswordInputRef.value?.inputRef, "Password must be longer than 6 characters");
        return false;
    }
    if (loginForm.value.confirmPassword !== loginForm.value.password) {
        setWarningMessage(confirmPasswordInputRef.value?.inputRef, "Passwords must match");
        return false;
    }
    return true;
}
